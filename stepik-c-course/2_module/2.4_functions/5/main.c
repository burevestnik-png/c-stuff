...
static int64_t i64_cmp(int64_t a, int64_t b) {
...
struct vm_state state_create(union ins *ip) {
...
void lift_unop(struct stack *s, int64_t (f)(int64_t)) {
    struct maybe_int64 head = stack_pop(s);

    if (head.valid) {
        stack_push(s, f(head.value));
    }
}
...
void lift_binop(struct stack *s, int64_t (f)(int64_t, int64_t)) {
    struct maybe_int64 first_arg = stack_pop(s);
    struct maybe_int64 second_arg = stack_pop(s);

    if (first_arg.valid && second_arg.valid) {
        stack_push(s, f(second_arg.value, first_arg.value));
    }
}
...
void interpret_iprint(struct vm_state *state) {
    ...
    if (popped.valid) {
        print_int64(popped.value);
    }
}

void interpret_iread(struct vm_state *state) {
    int64_t read_value = read_int64();
    stack_push(&state->data_stack, read_value);
}

void interpret_swap(struct vm_state *state) {
    struct maybe_int64 first_arg = stack_pop(&state->data_stack);
    struct maybe_int64 second_arg = stack_pop(&state->data_stack);

    stack_push(&state->data_stack, first_arg.value);
    stack_push(&state->data_stack, second_arg.value);
}
...
void interpret_print_instructions(struct vm_state *state) {
    printf("%s", instructions[state->ip->opcode].mnemonic);
    print_newline();
}

void interpret_print_stop(struct vm_state *state) {
    interpret_print_instructions(state);
    vm_stop(state);
}

void interpret_print_push(struct vm_state *state) {
    printf("%s ", instructions[state->ip->opcode].mnemonic);
...
static ins_interpreter *const ins_printers[] = {
        [BC_PUSH]   = interpret_print_push,
        [BC_IADD]   = interpret_print_instructions,
        [BC_ISUB]   = interpret_print_instructions,
        [BC_IMUL]   = interpret_print_instructions,
        [BC_IDIV]   = interpret_print_instructions,
        [BC_INEG]   = interpret_print_instructions,
        [BC_IPRINT] = interpret_print_instructions,
        [BC_IREAD]  = interpret_print_instructions,
        [BC_SWAP]   = interpret_print_instructions,
        [BC_POP]    = interpret_print_instructions,
        [BC_DUP]    = interpret_print_instructions,
        [BC_ICMP]   = interpret_print_instructions,
        [BC_STOP]   = interpret_print_stop
};
